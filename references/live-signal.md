...
### 买点三灯模式（--mode score，决策→跟踪闭环）

除信号策略外，模拟盘还可以直接按**买点三灯结论**纸面执行，追踪「三灯纪律本身」
的执行表现（无需 `--strategy`）：

```bash
# 每个交易日收盘后运行：跑三灯评分 → 按结论调仓 → 记录裁决历史
uv run python run_paper.py --symbol 600000.SH --mode score
```

结论→仓位映射：**趋势买点**=目标满仓；**纯趋势仓**=目标半仓（估值高，纪律
预设未经验证）；**回避 / 持仓需减风险**=目标空仓（按纪律离场，不等回本）；
**等回踩 / 左侧观察 / 无法评分**=维持现仓（不加仓不减仓）。持仓时会把近似成本
