...
四者互补：阶段说「在哪」→ 三灯说「能不能买」→ wisdom 规则说「历史上这么做行不行」。
...
| `breakout` | 突破确认 | 近 `confirm_days` 内收盘有效上穿「前置箱体」上沿 | 关键点出现，买卖交回三灯 |
...
`next_steps` 恒含无条件的 `score` 一项——**阶段只定位「在哪」，能不能买/买多少
一律交回三灯裁决**；另有条件项：`stage == breakout` → wisdom 规则回测、
`stage == advance` → 模拟盘跟踪、`stage == base` → 定投分批。
...
- **只看价量**：不含基本面与估值判断——`base` 不等于「便宜」，
  `advance` 不等于「值得买」。估值与硬伤请看三灯的「价」灯。
