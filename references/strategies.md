...
# 可选：金字塔分批加仓（试探性买入，对了再加码）
[pyramid]
units = 4     # 分批数 2~10，首批仓位 = 1/units
...
定义 `[pyramid]` 后，入场不再一次性满仓，而是试探性建仓、盈利后加码：

- 入场条件满足 → 先建 `1/units` 试探仓；
- 每当收盘价较上次加仓价顺方向浮盈 `step`，再加一批，直至满仓；
- 离场条件满足 → 一次性清仓（`allow_short` 时反手建空头试探仓，按下跌加仓）。

此时信号为分数仓位（如 0.25/0.5/0.75/1.0）；搭配 `--stop-loss` 时，
止损始终以首次入场价为基准。这是《炒股的智慧》「试探性买入、对了再
加码」原则的工程化，完整示例见 `examples/wisdom_rule.toml`。
