...
Level 5  进阶模块       →  多因子 / 配对 / 机器学习 / 新闻情绪 / 定投 / 事件研究 / 市场扫描
...
| 5 进阶 | 因子 / 配对 / ML / 情绪 / 定投 / 事件研究 / 扫描 | 「进阶模块」 | 完成 L2（建议先过 L4） |
...
## 进阶模块（Level 5：多因子 / 配对 / 机器学习 / 新闻情绪 / 定投 / 事件研究 / 市场扫描）
...
1. **筛池**：两条路径二选一或叠加——用财务指标筛优质股（见 [data-fetching.md](data-fetching.md) 的「筛选优质股票」），或用 `run_scan.py` 对股票池跑纪律评分漏斗，取「是/观察」档候选。
